...
export const adaptBooking = (dto: BookingDto): Booking =>
  ({
    ...
    packageType: (dto.packageType ?? 'UMRAH').toLowerCase() as 'hajj' | 'umrah',
    ...
    paymentStatus: paymentMap[dto.paymentStatus] ?? 'unpaid',
    ...
    notes: dto.notes ?? '',
    visaStatus: (dto.visaStatus ?? 'PENDING').toLowerCase() as Booking['visaStatus'],
    documentsStatus: (dto.documentsStatus ?? 'INCOMPLETE').toLowerCase() as Booking['documentsStatus'],
  }) as Booking
