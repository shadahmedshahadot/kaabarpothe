...
export default function AdminPilgrimsPage() {
  return (
    <>
      <PageTitle title="Pilgrims" description={`${pilgrims.length} registered pilgrims`} />
      <DataTable
        data={pilgrims}
        searchPlaceholder="Search pilgrims…"
        columns={[
          { key: 'p', label: 'Pilgrim', render: p => (
          ...
          { key: 'passport', label: 'Passport', render: p => (
            <div className="text-sm">
              <p className="font-mono text-foreground">{p.passport.number}</p>
              <p className="text-xs text-muted-foreground">Expires {formatDate(p.passport.expiryDate, { year: 'numeric', month: 'short' })}</p>
            </div>
          )},
          { key: 'bookings', label: 'Bookings', render: p => <span className="font-semibold text-foreground">{p.bookingsCount}</span> },
          { key: 'spent', label: 'Total spent', render: p => <span className="font-bold text-foreground">{formatCurrency(p.totalSpent)}</span> },
          { key: 'docs', label: 'Documents', render: p => {
            const verified = p.documents.filter(d => d.status === 'verified').length
            return <Badge variant={verified === p.documents.length ? 'success' : 'warning'}>{verified}/{p.documents.length} verified</Badge>
          }},
